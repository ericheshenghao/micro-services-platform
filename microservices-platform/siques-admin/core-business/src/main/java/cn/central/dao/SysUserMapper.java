...
//            "WHERE u.id = #{id,jdbcType=VARCHAR} AND u.id= ur.`user_id` AND ur.`role_id` = r.id"})
//    List<SysRole> findUserRolesById(String id);
//
//
...
//    IPage<SysUser> findUsersByOfficeId(IPage page, @Param(value = "id") String id);
//
//
//    @Select({"select u.* from  sys_user u ,sys_user_role ur where ur.`role_id`=#{id,jdbcType=VARCHAR} and ur.`user_id`=u.`id`\n"})
//    List<SysUser> findUsersByRoleId(String id);
